...
const CODE_TO_NAME: Record<string, string> = Object.fromEntries(
  Object.values(STATE_BY_FIPS).map((s) => [s.code.toLowerCase(), s.name]),
);
const NAME_TO_CODE: Record<string, string> = Object.fromEntries(
  Object.values(STATE_BY_FIPS).map((s) => [s.name.toLowerCase(), s.code]),
);
...
export function normalizeState(raw: string | null | undefined): {
  ...
  if (upper.length === 2 && CODE_TO_NAME[lower]) {
    return { code: upper, name: CODE_TO_NAME[lower] };
  }
  if (NAME_TO_CODE[lower]) {
    return { code: NAME_TO_CODE[lower], name: trimmed };
  }
  return null;
}
