...
function StatusBar({ business, phoneHref }: { business: ElectricalSiteData["business"]; phoneHref: string }) {
  return (
    <div className="border-b border-slate-900 bg-slate-950">
      <div className={`${MONO} mx-auto flex max-w-[1400px] items-center justify-between gap-4 px-6 py-2 text-[10.5px] uppercase tracking-[0.18em] text-white`}>
        ...
        {business.phone && (
          <a href={phoneHref} className="flex items-center gap-1.5 font-semibold text-white hover:text-cyan-300">
    ...
    <nav className="sticky top-0 z-30 border-b border-slate-200 bg-white/90 backdrop-blur-xl">
      <div className="mx-auto flex max-w-[1400px] items-center justify-between gap-6 px-6 py-4">
        <a href="#top" className="flex items-center gap-2.5">
          {business.logoUrl ? (
            <img src={business.logoUrl} alt={`${business.name} logo`} className="h-9 w-auto object-contain" />
          ) : (
            <span className="grid h-9 w-9 place-items-center rounded-sm bg-slate-950 text-cyan-400">
          ...
          <div className="flex flex-col leading-tight">
            ...
            <span className={`${MONO} text-[9px] uppercase tracking-[0.22em] text-cyan-700`}>
        ...
        <a
          href="#consult"
          className="inline-flex items-center gap-1.5 rounded-sm bg-slate-950 px-4 py-2 text-[12px] font-bold text-white transition hover:bg-slate-800"
        >
          Request dispatch <ArrowRight className="h-3 w-3" />
        </a>
      </div>
    </nav>
  );
...
function Hero({ data, phoneHref }: { data: ElectricalSiteData; phoneHref: string }) {
  ...
  return (
    <section id="top" className="relative overflow-hidden border-b border-slate-200 bg-white">
      <CircuitBackdrop />
      <div
        className="pointer-events-none absolute -left-40 top-0 h-[520px] w-[520px] rounded-full bg-cyan-400 opacity-[0.08] blur-3xl"
      />
      <div
        className="pointer-events-none absolute -right-40 bottom-0 h-[480px] w-[480px] rounded-full bg-amber-300 opacity-[0.10] blur-3xl"
      />

      <div className="relative mx-auto max-w-[1400px] px-6 pb-24 pt-16 lg:pb-32 lg:pt-20">
        <div className="grid gap-14 lg:grid-cols-[1.3fr_1fr] lg:items-end">
          <div>
            <div className={`${MONO} mb-8 inline-flex items-center gap-2 border border-cyan-600/30 bg-cyan-50 px-3 py-1.5 text-[10.5px] uppercase tracking-[0.24em] text-cyan-800`}>
              <Activity className="h-3 w-3" />
              {business.licenseNumber ?? "MASTER ELECTRICIAN"} · NEC 2023
            </div>

            <h1 className="max-w-3xl text-[clamp(2.75rem,7vw,6rem)] font-extrabold leading-[0.95] tracking-[-0.03em] text-slate-950">
              {hero.title}
            </h1>

            <p className="mt-8 max-w-xl text-[15px] leading-[1.65] text-slate-600">
              {hero.subtitle}
            </p>
            ...
            <div className="mt-10 flex flex-wrap items-center gap-3">
              <a
                href="#consult"
                className="group inline-flex items-center gap-2 bg-slate-950 px-7 py-4 text-[13px] font-bold text-white transition hover:bg-slate-800"
              >
                <Zap className="h-4 w-4 text-cyan-300" strokeWidth={2.5} />
                Request dispatch
              ...
              {business.phone && (
                <a
                  href={phoneHref}
                  className="inline-flex items-center gap-2 border border-slate-300 bg-white px-7 py-4 text-[13px] font-semibold text-slate-950 transition hover:border-slate-950"
                >
...
function LogoPanel({
  ...
  return (
    <div className="relative">
      <div className="absolute -right-3 -top-3 h-full w-full border border-cyan-400/60" />
      <div className="relative border border-slate-200 bg-white">
        <div className="grid h-[420px] w-full place-items-center bg-white p-12">
          {hasLogo ? (
            <img
            ...
            <div className="text-center">
              <span className="grid h-16 w-16 place-items-center rounded-sm bg-slate-950 text-cyan-400 mx-auto">
      ...
      <div className="mx-auto grid max-w-[1400px] grid-cols-2 divide-x divide-white/10 md:grid-cols-3 lg:grid-cols-6">
        {items.map((it) => (
          <div key={it.label} className="px-5 py-6">
            <div className={`${MONO} text-[9.5px] uppercase tracking-[0.24em] text-cyan-300`}>
              {it.label}
            </div>
            <div className={`${MONO} mt-2 text-lg font-bold text-white`}>
              {it.value}
            </div>
          </div>
        ))}
...
function Services({ services }: { services: ElectricalSiteData["services"] }) {
  return (
    <section id="services" className="relative border-b border-slate-200 bg-white">
      <div className="mx-auto max-w-[1400px] px-6 py-24 lg:py-28">
        <SectionHead
          numeral="01"
        ...
        />
        <div className="mt-14 grid gap-px border border-slate-200 bg-slate-200 md:grid-cols-2 lg:grid-cols-4">
          {services.map((s, i) => (
            <article
              key={s.title}
              className="group relative bg-white p-7 transition hover:bg-slate-50"
            >
              <div className="flex items-start justify-between">
                <span className="grid h-11 w-11 place-items-center border border-cyan-700/30 bg-cyan-50 text-cyan-700">
              ...
              <h3 className="mt-7 text-lg font-bold tracking-tight text-slate-950">{s.title}</h3>
  ...
  return (
    <section id="dispatch" className="relative border-b border-slate-200 bg-slate-50">
      <div className="mx-auto grid max-w-[1400px] gap-0 px-0 md:grid-cols-[1.1fr_1fr]">
        <div className="relative border-b border-slate-200 bg-white p-10 md:border-b-0 md:border-r md:p-14">
          <SectionHead
            numeral="02"
            eyebrow="Typical work order"
            title="100A → 200A panel upgrade."
          />
          <p className="mt-8 max-w-md text-[14.5px] leading-relaxed text-slate-600">
        ...
        <div className="relative min-h-[360px] bg-slate-100">
          {image ? (
            ...
            <div className="absolute inset-0 grid place-items-center">
              <div className="text-center">
                <CircuitBoard className="mx-auto h-16 w-16 text-cyan-600/60" strokeWidth={1.5} />
          ...
          <div className="absolute left-4 top-4">
            <span className={`${MONO} border border-cyan-600 bg-white/90 px-2.5 py-1 text-[9.5px] font-semibold uppercase tracking-[0.22em] text-cyan-700`}>
              WO #04210
            </span>
          </div>
        </div>
    ...
    <section id="process" className="border-b border-slate-200 bg-white">
      <div className="mx-auto max-w-[1400px] px-6 py-24 lg:py-28">
        ...
        <div className="relative mt-16">
          <div className="absolute left-0 right-0 top-[44px] hidden h-px bg-gradient-to-r from-transparent via-cyan-400 to-transparent md:block" />
          <div className="grid gap-8 md:grid-cols-4">
            {steps.map((s, i) => (
              <div key={s.step} className="relative">
                <div className="relative mx-auto grid h-[88px] w-[88px] place-items-center border border-cyan-600/60 bg-white">
                  <span className={`${MONO} text-2xl font-bold text-cyan-700`}>
                    {String(i + 1).padStart(2, "0")}
                  </span>
                  <span className="absolute -top-1 -right-1 h-2 w-2 bg-cyan-600" />
                  <span className="absolute -bottom-1 -left-1 h-2 w-2 bg-cyan-600" />
                </div>
                <h3 className="mt-6 text-center text-lg font-bold tracking-tight text-slate-950">
                  {s.title}
                </h3>
          ...
          {items.map((img, i) => (
            ...
            >
              ...
              <figcaption className="absolute bottom-3 left-3 right-3 flex items-center justify-between">
                <span className={`${MONO} text-[9.5px] font-semibold uppercase tracking-[0.22em] text-cyan-300`}>
                  JOB · {String(i + 1).padStart(3, "0")}
                </span>
                <span className={`${MONO} text-[9.5px] uppercase tracking-[0.18em] text-white/80`}>
      ...
      <div className="mx-auto max-w-[1400px] px-6 py-24 lg:py-28">
        <div className="grid gap-14 lg:grid-cols-[1fr_1.2fr]">
          ...
          <div className="space-y-6 border-l border-slate-200 pl-8">
            <p className="text-[15.5px] leading-[1.7] text-slate-700">{about}</p>
  ...
  return (
    <section id="credentials" className="border-b border-slate-200 bg-slate-50">
      <div className="mx-auto max-w-[1400px] px-6 py-24 lg:py-28">
        ...
        <div className="mt-14 grid grid-cols-2 gap-px border border-slate-200 bg-slate-200 md:grid-cols-4">
          {items.map((it) => (
            <div key={it.label} className="bg-white p-6">
              <div className={`${MONO} text-[10px] uppercase tracking-[0.24em] text-cyan-700`}>
                {it.label}
              </div>
              <div className={`${MONO} mt-3 text-lg font-bold text-slate-950`}>{it.value}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="mx-auto max-w-[1400px] px-6 py-24 lg:py-28">
        ...
        <div className="mt-14 grid gap-6 md:grid-cols-3">
          {testimonials.map((t, i) => (
            <blockquote
              key={i}
              className="relative flex flex-col border border-slate-200 bg-white p-7 shadow-[0_1px_0_rgba(15,23,42,0.03)]"
            >
              ...
              <p className="mt-5 flex-1 text-[14.5px] leading-relaxed text-slate-800">
                &ldquo;{t.quote}&rdquo;
              </p>
              <footer className="mt-6 flex items-center justify-between border-t border-slate-200 pt-4">
                <span className={`${MONO} text-[11px] font-semibold uppercase tracking-[0.14em] text-slate-950`}>
                  {t.author}
                </span>
                <span className={`${MONO} text-[9.5px] uppercase tracking-[0.22em] text-cyan-700/80`}>
                  {t.location}
                </span>
              </footer>
            </blockquote>
...
function Coverage({ cities, business }: { cities: string[]; business: ElectricalSiteData["business"] }) {
  return (
    <section className="border-b border-slate-200 bg-slate-50">
      <div className="mx-auto grid max-w-[1400px] gap-14 px-6 py-24 lg:grid-cols-[1fr_1.2fr] lg:py-28">
        <div>
          <SectionHead numeral="08" eyebrow="Service area" title="Who we dispatch to." />
        </div>
        <div>
          <div className="grid grid-cols-2 gap-x-8 gap-y-3 border-l border-slate-200 pl-8 md:grid-cols-3">
            {cities.map((c) => (
              <div key={c} className="flex items-center gap-2 text-[13.5px] text-slate-700">
          ...
          {business.phone && (
            <div className={`${MONO} mt-10 flex items-center gap-2 border-l border-slate-200 pl-8 text-[11px] uppercase tracking-[0.2em] text-slate-500`}>
              <Clock className="h-3 w-3 text-cyan-700" />
              Not listed? Call dispatch: {business.phone}
            </div>
          )}
        </div>
      </div>
  ...
  return (
    <section id="faq" className="border-b border-slate-200 bg-white">
      <div className="mx-auto max-w-[1400px] px-6 py-24 lg:py-28">
        ...
        <div className="mt-14 grid gap-px border border-slate-200 bg-slate-200 md:grid-cols-2">
          {faqs.map((f, i) => (
            <details
              key={i}
            ...
            >
              <summary className="flex cursor-pointer items-baseline justify-between gap-4 list-none">
                <div className="flex items-baseline gap-4">
                  <span className={`${MONO} text-[10px] font-semibold uppercase tracking-[0.22em] text-cyan-700`}>
                    Q.{String(i + 1).padStart(2, "0")}
                  </span>
                  <span className="text-[15px] font-semibold text-slate-950">{f.q}</span>
                </div>
                <PlusIcon className="h-4 w-4 shrink-0 text-slate-400 transition group-open:rotate-45" />
              </summary>
              <p className="mt-4 pl-10 text-[13.5px] leading-relaxed text-slate-600">{f.a}</p>
            </details>
          ))}
        </div>
      </div>
...
function Consult({ data }: { data: ElectricalSiteData }) {
  ...
  return (
    <section id="consult" className="relative overflow-hidden border-b border-slate-200 bg-white">
      <CircuitBackdrop />
      <div className="relative mx-auto grid max-w-[1400px] gap-14 px-6 py-24 lg:grid-cols-[1fr_1fr] lg:py-28">
        <div className="flex flex-col justify-center">
          <div className={`${MONO} inline-flex w-fit items-center gap-2 border border-emerald-600/40 bg-emerald-50 px-3 py-1.5 text-[10px] uppercase tracking-[0.24em] text-emerald-700`}>
            <span className="relative flex h-1.5 w-1.5">
              <span className="absolute inline-flex h-full w-full animate-ping rounded-full bg-emerald-500 opacity-75" />
              <span className="relative inline-flex h-1.5 w-1.5 rounded-full bg-emerald-500" />
            </span>
          ...
          <h2 className="mt-6 max-w-xl text-[clamp(2.25rem,5vw,4rem)] font-extrabold leading-[0.98] tracking-[-0.03em] text-slate-950">
          ...
          {business.phone && (
            <div className="mt-10 flex items-center gap-3 border-t border-slate-200 pt-6">
              ...
              <div>
                ...
                <a href={`tel:${business.phone.replace(/[^\d+]/g, "")}`} className="text-lg font-bold text-slate-950 hover:text-cyan-700">
                  {business.phone}
                </a>
              </div>
            </div>
    ...
    <footer className="bg-slate-950 text-white">
      <div className="mx-auto max-w-[1400px] px-6 py-16">
        <div className="grid gap-10 md:grid-cols-4">
          <div className="md:col-span-2">
            <div className="flex items-center gap-2.5">
              {business.logoUrl ? (
                ...
                <span className="grid h-8 w-8 place-items-center rounded-sm bg-cyan-400 text-slate-950">
                  <Zap className="h-4 w-4" strokeWidth={2.5} />
                </span>
              )}
              <span className="text-[15px] font-bold text-white">{business.name}</span>
            </div>
            <p className={`${MONO} mt-3 max-w-sm text-[10.5px] uppercase tracking-[0.22em] text-white/50`}>
              {business.licenseNumber ?? "Licensed · Bonded · Insured"}
            </p>
            {business.address && (
            ...
            {business.phone && (
              <a href={`tel:${business.phone.replace(/[^\d+]/g, "")}`} className="mt-2 flex items-center gap-2 text-[13px] text-white/70 hover:text-white">
                <Phone className="h-3.5 w-3.5 text-cyan-300" />
                {business.phone}
              </a>
            )}
            {business.email && (
              <a href={`mailto:${business.email}`} className="mt-1 flex items-center gap-2 text-[13px] text-white/70 hover:text-white">
          ...
          <div>
            <div className={`${MONO} text-[10px] uppercase tracking-[0.22em] text-cyan-300`}>Services</div>
          ...
          <div>
            <div className={`${MONO} text-[10px] uppercase tracking-[0.22em] text-cyan-300`}>Coverage</div>
            <ul className="mt-4 space-y-2 text-[13px] text-white/70">
        ...
        <div className="mt-14 flex flex-col items-start justify-between gap-4 border-t border-white/10 pt-6 text-[11px] text-white/50 md:flex-row md:items-center">
          <div className={MONO}>
...
function SectionHead({
  ...
  return (
    <div>
      <div className="flex items-baseline gap-4">
        <span className={`${MONO} text-[10px] font-semibold uppercase tracking-[0.24em] text-cyan-700`}>
          {numeral}
        </span>
      ...
      <h2 className="mt-5 max-w-3xl text-[clamp(2rem,4.2vw,3.75rem)] font-extrabold leading-[1] tracking-[-0.025em] text-slate-950">
        {title}
      </h2>
      {kicker ? (
        <p className="mt-5 max-w-xl text-[14.5px] leading-relaxed text-slate-600">{kicker}</p>
      ) : null}
    </div>
  );
}
