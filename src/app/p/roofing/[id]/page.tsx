...
// Industry-locked template for roofing contractors. Visual system centers on
// pitched-roof geometry and weather/resilience motifs — storm canvas over
// hero, ridge canvas with row-by-row shingle lay-in, material catalog by
// roof type, warranty tiers, inspection report, license/manufacturer
// certifications. Not usable for non-roofing businesses (enforced in
// site-url.ts via INDUSTRY_LOCK).
export default async function RoofingHomePage({
  ...
  const plans = buildWarrantyPlans();
  // Stats derived from scraped review volume. Keeps the numbers
  // site-specific without fabricating claims we can't back up.
  const roofsInstalled = Math.max(320, business.reviewsCount * 6 + 480);
  const sqFtReplaced = Math.max(1_800_000, business.reviewsCount * 4_200 + 2_100_000);
  const warrantyYears = 50;

  return (
    <div className="relative min-h-screen overflow-hidden bg-[#0b1220] text-slate-100">
      {/* Masthead */}
      <header className="relative z-20 border-b border-slate-800/80 bg-[#0b1220]/90 backdrop-blur">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-5 py-4 sm:px-8">
          <Link
            href={`/p/roofing/${data.slug}`}
          ...
          <div className="hidden items-center gap-6 text-[11px] uppercase tracking-[0.22em] md:flex">
            <a href="#materials" className="text-slate-400 hover:text-slate-100">
              Materials
            </a>
            <a href="#warranty" className="text-slate-400 hover:text-slate-100">
              Warranty
            </a>
            <a href="#coverage" className="text-slate-400 hover:text-slate-100">
              Coverage
            </a>
            {loc && <span className="text-slate-500">{loc}</span>}
            {business.phone && (
              <a
      ...
      {/* Hero — storm + ridge canvas + serif title split */}
      <section className="relative isolate overflow-hidden">
        ...
        {/* Ridge canvas (back layer, offset right) */}
        <div className="absolute inset-0 opacity-70">
          <RoofRidgeCanvas />
        </div>
        {/* Storm overlay (front layer, muted so rain reads but doesn't dominate) */}
        <div className="absolute inset-0 opacity-80">
          <StormCanvas density={100} />
        </div>

        <div className="relative mx-auto max-w-7xl px-5 pb-24 pt-28 sm:px-8 sm:pb-32 sm:pt-36">
          <p className="font-serif text-[13px] italic tracking-wide text-amber-300/90">
            {hero.eyebrow || "Roofing, built to outlast the weather"}
          </p>
          <h1 className="mt-4 max-w-4xl font-sans text-[44px] font-black leading-[0.95] tracking-tight text-slate-50 sm:text-[64px] md:text-[80px]">
            {(() => {
          ...
          <p className="mt-6 max-w-2xl text-[15px] leading-relaxed text-slate-300 sm:text-[17px]">
            {hero.subtitle}
          </p>
          ...
          <div className="mt-10 flex flex-wrap items-center gap-3">
            ...
            {rating != null && (
              <span className="inline-flex items-center gap-1.5 text-[12px] font-semibold uppercase tracking-[0.16em] text-slate-400">
                ...
                {rating.toFixed(1)} · {business.reviewsCount.toLocaleString()} reviews
              </span>
            )}
          </div>
        </div>
          ...
          <div className="relative">
            <div className="relative aspect-[4/5] overflow-hidden rounded-2xl border border-slate-800">
              {gallery[0]?.src ? (
                <SafeImg
                  src={gallery[0].src}
                  alt={gallery[0].alt || `${business.name} installation`}
      ...
      {/* Before / After (if we have gallery pairs) */}
      {gallery.length >= 2 && (
        <section className="relative">
          <div className="mx-auto max-w-7xl px-5 py-20 sm:px-8">
            <p className="text-[11px] font-semibold uppercase tracking-[0.26em] text-amber-400">
            ...
            <div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {gallery.slice(0, 6).map((g, i) => (
                ...
                >
                  <SafeImg
                    src={g.src}
                    alt={g.alt || `${business.name} project`}
                    className={`h-full w-full object-cover ${
      ...
      {/* Footer */}
      <footer className="relative border-t border-slate-800/80 bg-[#070c16]">
        <div className="mx-auto grid max-w-7xl gap-8 px-5 py-12 text-[12.5px] text-slate-400 sm:grid-cols-3 sm:px-8">
          <div>
...
// Splits a hero headline on a sentinel (": " or " — ") so we can typeset the
// first part in serif-italic and the rest in bold sans. Falls back to a single
// sans line when no natural split exists.
...
// Deterministic seed from a string — used for placing coverage map pins so
// they're stable across renders but organically distributed.
