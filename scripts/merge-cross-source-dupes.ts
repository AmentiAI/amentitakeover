/**
 * Merges cross-source duplicates: rows with the same apex domain AND
 * same lowercase city, regardless of which source brought them in.
 * Keeps the row with the most data (audited > not, has email > not,
 * source priority: google > pestworld > osm-pest-control > others) and
 * folds contact-info and phone history from siblings into the kept row,
 * then archives the siblings.
 *
 *   tsx scripts/merge-cross-source-dupes.ts            # dry-run
 *   tsx scripts/merge-cross-source-dupes.ts --commit
 */
...
function rankRow(r: {
  ...
  // tie-break: older row wins (preserves history)
...
async function main() {
  ...
  // Group by (apex-domain, lowercase city). City must exist for grouping
  // — Orkin-style chain rows where city differs across rows naturally
  // fall into different groups so they aren't merged.
  ...
  for (const r of rows) {
    ...
    if (!c) continue; // skip rows without a city — too risky to auto-merge
    const key = `${a}|${c}`;
  ...
  console.log(`Found ${dupeGroups.length} cross-source duplicate groups`);
  let toArchive: { id: string; reason: string }[] = [];
  let toUpdate: { id: string; data: Prisma.ScrapedBusinessUpdateInput }[] = [];
  ...
  for (const [key, list] of dupeGroups) {
    ...
    // Merge phones audit trail from losers into winner.
    ...
    for (const r of [winner, ...losers]) {
      ...
      else if (r.phone) pushPhone({ number: r.phone, source: r.source, scrapedAt: r.createdAt.toISOString() });
    }

    // Roll up email if winner is missing one.
    ...
    for (const l of losers) {
      toArchive.push({ id: l.id, reason: `merged into ${winner.id} (${key})` });
    }

    console.log(
      `  ${key}  keep=[${winner.source}] ${winner.name}  drop=${losers.length} (${losers.map((l) => l.source).join(", ")})`,
    );
  }
